...
export default function TimersPage() {
  ...
  const createTimer = (type = 'stopwatch') => {
    const initialTime = type === 'countdown' ? prompt('Enter initial time in minutes:') * 60000 : 0
    ...
    setTimers(timers.map(timer => {
      if (timer.id === id) {
        if (!timer.isRunning && timer.type === 'countdown' && timer.time <= 0) {
          return { ...timer, time: timer.countdownStart, isRunning: !timer.isRunning }
        }
  ...
  const resetTimer = (id) => {
    setTimers(timers.map(timer => 
      timer.id === id 
        ? { ...timer, time: timer.type === 'countdown' ? timer.countdownStart : 0, laps: [], isRunning: false } 
        : timer
    ))
  }
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'} p-8`}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {timers.map(timer => (
            <div key={timer.id} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-6 shadow-sm`}>
                ...
                <div className="mt-4 space-y-2">
                  ...
                  {timer.laps.map((lap, index) => (
                    <div key={index} className="flex justify-between">
                      ...
                      <span>{formatTime(lap, globalHidden || timer.hidden)}</span>
                    </div>
                  ))}
                </div>
              )}
